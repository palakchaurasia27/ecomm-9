...
public class ProductController {
	...
	public Product updateProduct(@RequestBody Product productObj) {
		// step 1: find product
		Product fetchedProduct = pRepo.findById(productObj.getProductId()).orElseThrow(() -> {
			throw new ProductNotFoundException("Product does not exist with id " + productObj.getProductId());
		});
		// step 2: Map updating fields
		fetchedProduct.setProductName(productObj.getProductName());
		fetchedProduct.setProductDesc(productObj.getProductDesc());
		fetchedProduct.setProductColour(productObj.getProductColour());
		fetchedProduct.setProductSize(productObj.getProductSize());
		fetchedProduct.setProductPrice(productObj.getProductPrice());
		// step 3: update
